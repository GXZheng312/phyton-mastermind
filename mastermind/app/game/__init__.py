...
class GameObject:
    ...
    def append_history(self, blocks):
        ...
        for n in range(len(self.board.solution)):
            ...
            if blocks[n] == self.board.solution[n]:
                block_item.black_pin = True
            elif blocks[n] in self.board.solution:
                block_item.white_pin = True
...
class Board:
    ...
    def setup_board(self, amount_color, multi):
        ...
        for i in range(0, self.positions):
            self.solution.append(available_colors.pop(
                random.randrange(0, len(available_colors))))
